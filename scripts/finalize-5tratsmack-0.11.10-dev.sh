...
replace_once SOURCE_SHA12_REQUIRED "$short_revision" "$compose"
...
grep -Fx "    image: ghcr.io/willitmod/5tratsmack-app:0.11.10-rc.${short_revision}@${app_digest}" "$compose" >/dev/null
grep -Fx "    image: ghcr.io/willitmod/5tratsmack-ckpool:0.11.3-rc.${short_revision}@${ckpool_digest}" "$compose" >/dev/null
grep -Fx "      APP_IMAGE: ghcr.io/willitmod/5tratsmack-app:0.11.10-rc.${short_revision}@${app_digest}" "$compose" >/dev/null
...
grep -Fx "      CKPOOL_IMAGE: ghcr.io/willitmod/5tratsmack-ckpool:0.11.3-rc.${short_revision}@${ckpool_digest}" "$compose" >/dev/null
